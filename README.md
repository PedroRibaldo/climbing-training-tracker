...
- Join one or more gyms from the sidebar profile panel; gym admins can manage members' roles from the same panel — the identity foundation for gym-specific features.
...
│   ├── models.py
│   └── store.py
├── gyms/                        # Gym entities and per-gym membership/roles
│   ├── models.py
│   └── store.py
...
Eight tables - a session can reference any number of exercises without repeating data, a goal drives the sessions it generates, gyms/memberships give a user per-gym roles, and every user's data is isolated by Postgres Row-Level Security rather than app-code filtering alone.
...
-- Gyms are only ever created manually (service-role key / SQL editor) -
-- there's no in-app gym-creation flow.
CREATE TABLE gyms (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    name TEXT NOT NULL,
    location TEXT,
...
CREATE POLICY "select gyms" ON gyms FOR SELECT TO authenticated USING (true);

-- One row per (user, gym)
CREATE TABLE gym_memberships (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE DEFAULT auth.uid(),
    gym_id BIGINT NOT NULL REFERENCES gyms(id) ON DELETE CASCADE,
    role TEXT NOT NULL DEFAULT 'climber'
        CHECK (role IN ('climber', 'worker', 'setter', 'trainer', 'competitor', 'admin')),
...
AS $$
  SELECT EXISTS (
    ...
    WHERE gym_id = target_gym_id AND user_id = auth.uid() AND role = 'admin'
  );
$$;

CREATE POLICY "select own memberships" ON gym_memberships FOR SELECT
    USING (auth.uid() = user_id OR is_gym_admin(gym_id));
CREATE POLICY "join gym" ON gym_memberships FOR INSERT
    WITH CHECK (auth.uid() = user_id AND role = 'climber');
CREATE POLICY "admin updates member role" ON gym_memberships FOR UPDATE
    USING (is_gym_admin(gym_id)) WITH CHECK (is_gym_admin(gym_id));
```
